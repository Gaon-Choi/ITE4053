...
def generate_dataset(m, n):
    ...
    x_train = []; y_train = []
    for i in range(m):
        new_data = np.array([random.uniform(-10, 10), random.uniform(-10, 10)])
...
def vectorwise_learning(x_train, y_train, x_test, y_test, k, w, b, lr):
    ...
    for _ in range(k):
        ...
        # if (_+1) % 10 == 0:
        #     print("W : {w1} {w2}".format(w1=w[0], w2=w[1]))
...
def set_parser(parser):
    ...
    parser.add_argument('mode', type=str, choices=['time', 'W', 'weights', 'alpha'])
...
def main():
    ...
    w_elementwise = np.array([r1, r2])
    b_elementwise = r3

    # weights for vector-wise learning
    w_vectorize = np.array([r1, r2])
    b_vectorize = r3
    ...
    if args.mode == 'W':
        for _ in range(400):
            ...
            tmp1 = w_elementwise
            tmp2 = b_elementwise
            model = vectorwise_learning(x_train, y_train, x_test, y_test, args.k, tmp1, tmp2, 0.01)
