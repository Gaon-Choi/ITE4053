...
def main():
    ...
    w1 = np.random.uniform(-1, 1, (HH, XX))
    b1 = np.random.uniform(-1, 1, (HH, XX))

    w2 = np.random.uniform(-1, 1, (YY, HH))
    ...
    if args.mode == 'W':
        ...
        for _ in range(100):
            ...
            ax1.plot(model[0][0][0], model[0][1][0], '.')
            ax2.plot(model[2][0][0], model[2][0][1], '.')
            ax3.plot(model[1][0][0], model[1][1][0], '.')
            B2.append(model[3][0][0])
        ...
        ax3.set_title('B1')
        ax4.set_title('B2')
        ax4.hist(B2, bins=100, color='red', edgecolor='black')
